...
/-----------------------------------------------------------------------------------------------------*/
try {
	logDebug("ACA ONLOAD EQUIP LIST");
		...
		if (parentCapId == null) {
			...
			if (equipTable && equipTable.length > 0) {
				...
				for (var eachRow in equipTable) {
					...
					newRow["Vehicle Action"] = new asiTableValObj("Vehicle Action", "No Change", "N");
					...
					if (thisRow["Plate Fee"] == null) 
						newRow["Plate Fee"] = new asiTableValObj("Plate Fee", "", "N");
					else
